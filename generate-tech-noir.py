#!/usr/bin/env python3
"""Generate tech-noir versions for the verse-to-prompt demo.

Reads attempts-tech-noir.json, generates images via fal.ai z-image/turbo,
and produces a standalone manifest for tech-noir.html.

Same model + same seed as the original demo — prompts are the only variable.
"""
...
def main():
    cfg = json.loads((ROOT / "attempts-tech-noir.json").read_text())
    verses = cfg["verses"]
    key = fal_key()
    ...
    (ROOT / "manifest-tech-noir.js").write_text(
        "window.MANIFEST_TECH_NOIR = " + json.dumps(manifest, ensure_ascii=False, indent=2) + ";\n")
    print(f"\nWrote manifest-tech-noir.js ({len(manifest)} attempts). Open tech-noir.html to view.")
